...
using namespace std;
    ...
    Jet::Jet(const ModelProto &model_data) {
      ...
      btn_roll_left = btn_roll_right =
	btn_yaw_left = btn_yaw_right =
	btn_pitch_up = btn_pitch_down = false;
...
//       temprotshield = 0;
     ...
     Jet::~Jet() {
       cout<<"+-+ Deleting Jet\n| ";
       if (thruster!=NULL) { delete thruster; thruster = NULL; cout<<"| ";}
...
//       glEnd();
  ...
  void  Jet::Update() {
    ...
    if (btn_roll_left) {
      if (rotZ<maxangleZ) { rotZ += anglespeedZ*DELTATIME; } }
    if (btn_roll_right) {
      if (rotZ>-maxangleZ) { rotZ -= anglespeedZ*DELTATIME; } }
    if ((!btn_roll_left && !btn_roll_right) || (btn_roll_left && btn_roll_right)) { 
      roll = 0;   
      if (rotZ<-0.1*DELTATIME) { rotZ += anglespeedZ*DELTATIME; }
      if (rotZ>0.1*DELTATIME) { rotZ -= anglespeedZ*DELTATIME; }
    } 
    else { if (btn_roll_right) { roll = 60; } else { roll = -60; }}
    
    
    
    if (btn_yaw_left) {
      if (rotY<maxangleY) { rotY += anglespeedY*DELTATIME; } }
    if (btn_yaw_right) {
      if (rotY>-maxangleY) { rotY -= anglespeedY*DELTATIME; } }
    if ((!btn_yaw_left && !btn_yaw_right) || (btn_yaw_left && btn_yaw_right)) { 
      yaw = 0; 
      if (rotY<-0.1*DELTATIME) { rotY += anglespeedY*DELTATIME; }
      if (rotY>0.1*DELTATIME) { rotY -= anglespeedY*DELTATIME; }
    } 
    else { if (btn_yaw_right) { yaw = 20; } else { yaw = -20; }}
    
    
    if (btn_pitch_up) {
      if (rotX<maxangleX) { rotX += anglespeedX*DELTATIME; } }
    if (btn_pitch_down) {
      if (rotX>-maxangleX) { rotX -= anglespeedX*DELTATIME; } }
    if ((!btn_pitch_up && !btn_pitch_down) || (btn_pitch_up && btn_pitch_down)) { 
      pitch = 0;
      if (rotX<-0.1*DELTATIME) { rotX += anglespeedX*DELTATIME; }
      if (rotX>0.1*DELTATIME) { rotX -= anglespeedX*DELTATIME; }
    } 
    else { if (btn_pitch_up) { pitch = 40; } else { pitch = -40; }}
...
//     }
    ...
    if ((speed<max_speed) && (speed_acc>0)) { speed += 0.8*DELTATIME; }
      else { if ((speed>min_speed) && (speed_acc<0)) { speed -= 0.8*DELTATIME; } }
    if (((speed>=norm_speed+0.1*DELTATIME) || (speed<=norm_speed-0.1*DELTATIME)) && (speed_acc==0)) { 
      if (speed<norm_speed) { speed += 0.3*DELTATIME; } else { speed -= 0.3*DELTATIME; }
    }
    if (speed<3) { thruster->SetLength(speed/8); }
...
//     } else { roll = 0; }
      ...
      if (shotfactor[0]<Weapon[0].delay) { shotfactor[0]+=1.0*DELTATIME; }
      if (shotfactor[1]<Weapon[1].delay) { shotfactor[1]+=1.0*DELTATIME; }
      if (shotfactor[2]<Weapon[2].delay) { shotfactor[2]+=1.0*DELTATIME; }
      if (energy<100) { energy+=60*DELTATIME; } else if (energy>100) { energy = 100; }
    ...
    bool Jet::IsWeaponReady(GLuint WeaponNum) {
      if ((shotfactor[WeaponNum] >= Weapon[WeaponNum].delay) &&
        (energy>=Weapon[WeaponNum].energy) ) { return true; }
      return false;
    }
