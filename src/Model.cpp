...
//         cout<<" "<<line.c_str()<<"\n";
      if (line.substr(0,2) == "o ") {
        line.erase(0,2);
        if (strcmp(line.c_str(),"hull")==0) {dataType=1; } 
         else { if (strcmp(line.c_str(),"shield")==0) {dataType=2; } 
          else { if (strcmp(line.c_str(),"weapons")==0) {dataType=3; } 
           else { if (strcmp(line.c_str(),"thruster")==0) {dataType=4; } 
            else { dataType=0; }}}}
      }
        ...
        switch(dataType){
          case 1: break;
          case 2: break;
          case 3: if (wID<3) { weapons[wID] = tmpv; wID++; } break;
          case 4: thrusters.push_back(tmpv); break;
          default: break;
        }
      }
  ...
  void Model::NormalizeSize() {
    ...
    for (k=0; k<faces.size(); k++) {
      for (l=0; l<faces[k].vertex.size(); l++){
        if (faces[k].vertex[l].z>maxZ) { maxZ=faces[k].vertex[l].z; }
        if (faces[k].vertex[l].z<minZ) { minZ=faces[k].vertex[l].z; }
      }
    }
