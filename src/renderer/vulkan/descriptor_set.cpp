...
DescriptorSet::DescriptorSet( VkDevice device, uint32_t swapchainCount, const std::pmr::vector<std::pair<VkDescriptorType, VkShaderStageFlagBits>>& types )
: m_device{ device }
{
    ...
    for ( size_t i = 0; i < types.size(); ++i ) {
        ...
        descriptorPoolSize[ i ].descriptorCount = swapchainCount;
    }
    ...
    m_set.resize( swapchainCount );
    const std::pmr::vector<VkDescriptorSetLayout> layouts( swapchainCount, m_layout );
...
VkDescriptorSet DescriptorSet::operator [] ( size_t i )
{
    return m_set[ i ];
}
