...
void Button::Draw() {
  ...
  glPushMatrix();
    ...
    if (enabled) { glColor3f(0,0.75,1); }
      else { glColor3f(0.3, 0.55, 0.65); }
    glBegin(GL_QUADS);
...
void Button::SetTexture(GLuint t) { textureID = t; }
...
void Button::SetFont(Font *F) {
  font = F;
  if (font!=NULL) { text_length = font->GetTextLength(text.c_str())/2; }
}
Button::~Button() {};

void Button::Enable(const bool &B) { enabled = B; }
