...
void Engine::loadDDS( Asset&& asset )
{
    ...
    if ( !asset.read( header ) ) {
        assert( !"not enough data to read dds header" );
        return;
    }
    ...
    if ( header.pixelFormat.fourCC != dds::DXGI ) {
        assert( !"only DX10 DDS can be loaded" );
        return;
    }

    dds::dxgi::Header dxgiHeader{};
    if ( !asset.read( dxgiHeader ) ) {
        assert( !"not enough data to read DXGI subheader" );
        return;
    }
    ...
    Texture tex = m_renderer->createTexture( tci, asset.data );
    assert( tex );
